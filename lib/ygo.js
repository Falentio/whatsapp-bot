...
async function search(name,  withPic){
  ...
  if(length === 1 && withPic)return [await dataFilter(cardData.data[0], true),await getPic(cardData.data[0])]
  if(length === 1)return cardInfo = await dataFilter(cardData.data[0], true)
...
async function getWName(name, withPic){
